...
function Composer({ agent, onSend, openMenu, menuOpen, onPickAgent, mini, disabled,
                    where, attached, onRemoveAttach, attachFlow,
                    files, attachedIds, attachOpen, setAttachOpen, onToggleAttachFile, onUpload }) {
  const { Icons } = window;
  const { FileGlyph, AttachMenu, fx } = window.FilesKit;
  const ref = useRef(null);
  const [hasText, setHasText] = useState(false);
  const canSend = (hasText || attached.length > 0) && !disabled;
  const send = () => {
    const txt = (ref.current.textContent || "").trim();
    if ((!txt && attached.length === 0) || disabled) return;
    ref.current.textContent = "";
    setHasText(false);
    onSend(txt);
  };
  const onKey = (e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } };
  const openHere = attachOpen === where;
  const toggleAttach = (e) => { e.stopPropagation(); setAttachOpen(openHere ? null : where); };
  return (
    <div className="composer" style={mini ? { boxShadow: "none" } : null}>
      {attached.length > 0 && (
        <div className="attach-pills">
          {attached.map((f) => (
            <div key={f.id} className={"apill" + (f.uploading ? " uploading" : "") + (f.justdone ? " justdone" : "")}>
              <FileGlyph name={f.name} size={22} />
              <span className="ap-name">{f.name}</span>
              {f.uploading
                ? <span className="ap-prog"><i /></span>
                : <span className="ap-size">{fx.fmtSize(f.bytes)}</span>}
              <span className="ap-x" onClick={() => onRemoveAttach(f.id)} title={f.uploading ? "Cancel" : "Remove"}><Icons.x s={14} /></span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="composer-row">
        <div style={{ position: "relative" }}>
          <button className="cbtn" style={openHere ? { background: "var(--surface-2)", color: "var(--text)" } : null} onClick={toggleAttach} title="Attach files"><Icons.attach s={17} /></button>
          {attachFlow === "popover" && openHere && (
            <AttachMenu files={files} attachedIds={attachedIds} onAttach={onToggleAttachFile} onUpload={onUpload}
              style={{ bottom: "calc(100% + 10px)", left: 0 }} />
          )}
        </div>
        <span className="spacer" />
        <button className="cbtn" style={{ minWidth: 34, padding: 0 }}><Icons.mic s={17} /></button>
        <button className={"send" + (canSend ? "" : " idle")} onClick={send}><Icons.arrowUp s={18} /></button>
      </div>
    </div>
  );
}
...
/* ---- sidebar ---- */
function Sidebar({ recents, onNew, onOpenRecent, activeRecent, onToggle, view, onOpenFiles, fileCount }) {
  const { Icons } = window;
  ...
  return (
    <aside className="sb">
      ...
      <div className="sb-body">
        ...
        <nav className="nav">
          <div className={"nav-item" + (inChat ? " active" : "")} onClick={(e) => { e.stopPropagation(); onOpenFiles(false); }}><span className="ic"><Icons.chat s={17} /></span> Sessions</div>
          <div className="nav-item"><span className="ic"><Icons.search s={17} /></span> Search</div>
          <div className={"nav-item" + (view === "files" ? " active" : "")} onClick={(e) => { e.stopPropagation(); onOpenFiles(true); }}>
            <span className="ic"><Icons.folder s={17} /></span> Files
            <span className="spacer" />
            <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-3)" }}>{fileCount}</span>
          </div>
...
function App() {
  ...
  const [files, setFiles] = useState(() => window.FilesKit.seedFiles());
  const [attached, setAttached] = useState([]);      // files on the current draft
  const [attachOpen, setAttachOpen] = useState(null); // which composer's attach UI is open
  const [toast, setToast] = useState(null);          // { file, idx } for undo
  ...
  /* ---- attach / upload / files handlers ---- */
  const toggleAttachFile = (f) => setAttached((a) => a.some((x) => x.id === f.id) ? a.filter((x) => x.id !== f.id) : [...a, f]);
  const removeAttach = (id) => setAttached((a) => a.filter((x) => x.id !== id));
  const attachFromFiles = (f) => { setAttached((a) => a.some((x) => x.id === f.id) ? a : [...a, f]); setView("chat"); };

  const openUpload = () => { setAttachOpen(null); fileInputRef.current && fileInputRef.current.click(); };
  const onFilePicked = (e) => {
    const picked = Array.from(e.target.files || []);
    picked.forEach((file, i) => {
      const id = "u" + Date.now() + "_" + i;
      const entry = { id, name: file.name, bytes: file.size, added: Date.now(), source: null, uploading: true };
      setAttached((a) => [...a, entry]);
      setTimeout(() => {
        setAttached((a) => a.map((x) => x.id === id ? { ...x, uploading: false, justdone: true } : x));
        setFiles((fs) => [{ id, name: file.name, bytes: file.size, added: Date.now(), source: null }, ...fs]);
        setTimeout(() => setAttached((a) => a.map((x) => x.id === id ? { ...x, justdone: false } : x)), 760);
      }, 1300 + i * 280);
    });
    e.target.value = "";
  };
  ...
  const renameFile = (id, name) => {
    ...
    setAttached((a) => a.map((f) => f.id === id ? { ...f, name } : f));
  };
  const deleteFile = (f) => {
    const idx = files.findIndex((x) => x.id === f.id);
    setFiles((fs) => fs.filter((x) => x.id !== f.id));
    setAttached((a) => a.filter((x) => x.id !== f.id));
    setToast({ file: f, idx, key: Date.now() });
  };
  const undoDelete = () => {
    if (!toast) return;
    setFiles((fs) => { const c = [...fs]; c.splice(Math.min(toast.idx, c.length), 0, toast.file); return c; });
    setToast(null);
  };
  useEffect(() => {
    if (!toast) return;
  ...
  const openFiles = (isFiles) => { setView(isFiles ? "files" : "chat"); setMenuOpen(null); setAttachOpen(null); };
  ...
  const send = (text) => {
    if (pending) return;
    const carried = attached.filter((f) => !f.uploading);
    setMessages((m) => [...m, { role: "user", text, time: nowTime(), files: carried }]);
    setAttached([]);
    setAttachOpen(null);
    setView("chat");
    run(agentId);
  ...
  const newSession = () => { setMessages([]); setPending(null); setActiveRecent(null); setMenuOpen(null); setAttached([]); setAttachOpen(null); setView("chat"); setSessionId((s) => s + 1); };
  const openRecent = (i) => {
    const r = RECENTS[i];
    setActiveRecent(i); setAgentId(r.agent); setMenuOpen(null); setAttachOpen(null); setView("chat");
  ...
  return (
    <div className="hexa app" data-theme={t.theme} data-density={t.density} data-font={t.face} data-accent={dataAccent}
         style={accentStyle} onClick={() => { setMenuOpen(null); setAttachOpen(null); }}>
      {sidebarVisible && (
        <Sidebar recents={RECENTS} onNew={newSession} onOpenRecent={openRecent} activeRecent={activeRecent}
                 onToggle={() => setCollapsed(true)} view={view} onOpenFiles={openFiles} fileCount={files.length} />
      )}
      ...
      <div className="main">
        <div className="topbar">
          ...
          {view === "files"
            ? <span style={{ color: "var(--text-3)", fontSize: 14 }}>· Files</span>
        ...
        {view === "files" ? (
          <window.FilesKit.FilesView files={files} attachedIds={attachedIds}
            onAttach={attachFromFiles} onRename={renameFile} onDelete={deleteFile} onUpload={openUpload} />
        ) : !inChat ? (
          <div className="stage" style={{ alignItems: "stretch" }}>
            <div style={{ width: "100%", maxWidth: 720, margin: "0 auto" }}>
              ...
              <Composer agent={agent} onSend={send} menuOpen={menuOpen} disabled={!!pending}
                        ...
                        where="hero" attached={attached} onRemoveAttach={removeAttach} attachFlow={t.attach}
                        files={files} attachedIds={attachedIds} attachOpen={attachOpen} setAttachOpen={setAttachOpen}
                        onToggleAttachFile={toggleAttachFile} onUpload={openUpload} />
              ...
              return (<>
                    ...
                    <div className="msg-body" style={{ display: "flex", justifyContent: "flex-end" }}>
                      <div className="ucard">
                        {m.files && m.files.length > 0 && (
                          <div className="ucard-files">
                            {m.files.map((f) => (
                              <div key={f.id} className="ufile">
                                <window.FilesKit.FileGlyph name={f.name} size={20} />
                                <span>{f.name}</span>
                                <span className="uf-size">{window.FilesKit.fx.fmtSize(f.bytes)}</span>
                              </div>
                            ))}
                          </div>
                        )}
                ...
                <div className="thread-foot">
                  <Composer agent={agent} onSend={send} mini menuOpen={menuOpen} disabled={!!pending}
                            openMenu={(w) => setMenuOpen(w)} onPickAgent={pickAgent}
                            where="foot" attached={attached} onRemoveAttach={removeAttach} attachFlow={t.attach}
                            files={files} attachedIds={attachedIds} attachOpen={attachOpen} setAttachOpen={setAttachOpen}
                            onToggleAttachFile={toggleAttachFile} onUpload={openUpload} />
                </div>
              </>);
      ...
      {/* ---- attach palette (variation B) + upload input + undo toast ---- */}
      <input ref={fileInputRef} type="file" multiple style={{ display: "none" }}
             accept=".pdf,.csv,.tsv,.txt,.md,.json,.yaml,.yml,.js,.ts,.py,.sh,.sql" onChange={onFilePicked} />
      {t.attach === "palette" && attachOpen && (
        <window.FilesKit.AttachPalette files={files} attachedIds={attachedIds}
          onAttach={toggleAttachFile} onUpload={openUpload} onClose={() => setAttachOpen(null)} />
      )}
      {toast && (
        <div className="f-toast" key={toast.key} onClick={(e) => e.stopPropagation()}>
          <span className="ft-msg">Deleted <b>{toast.file.name}</b></span>
          <span className="ft-undo" onClick={undoDelete}><Icons.refresh s={14} /> Undo</span>
          <span className="ft-meter"><i /></span>
        </div>
      )}
      ...
      <TweaksPanel>
        ...
        <TweakRadio label="Attach flow" value={t.attach} options={["popover", "palette"]} onChange={(v) => { setTweak("attach", v); setAttachOpen(null); }} />
